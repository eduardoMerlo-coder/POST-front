            ...
            return (
              <div
                ...
                className={`relative bg-surface border-1 rounded-lg p-3 transition-colors ${
                  isSelected
                    ? "border-[#00C89C] bg-surface-alt"
                    : "border-border hover:border-accent hover:bg-surface-alt cursor-pointer"
                }`}
                onClick={!isSelected ? () => onAddProduct(product) : undefined}
              >
                {isSelected && (
                  <button
                    onClick={(e) => {
                    ...
                    className="absolute -top-3 -left-3 w-6 h-6 flex items-center justify-center bg-accent hover:bg-accent/90 text-white rounded-full transition-colors z-10"
                ...
                {isSelected && (
                  <div className="absolute top-2 right-2 min-w-[1.5rem] px-1 flex items-center justify-center bg-[#00C3A0]/90 text-black rounded-full text-xs font-bold z-10 shadow-lg">
                ...
                <div className="flex flex-col gap-2">
                  <h3
                    className={`font-semibold text-sm text-primary line-clamp-2 ${
                ...
                {isSelected && (
                  <button
                    ...
                    className="absolute inset-0 flex items-center justify-center rounded-lg bg-[#00C3A0]/10 hover:bg-[#00C3A0]/20 transition-colors z-0"
                    aria-label="Incrementar cantidad"
                  >
                    <div className="w-10 h-10 flex items-center justify-center rounded-full shadow-lg">
                      <FaPlus className="text-[#00C3A0]" size={20} />
                    </div>
                  </button>
                )}
              </div>
